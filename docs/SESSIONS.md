...
# Phase4-D4g 交付报告 — 数账清查（恢复提交内含物 + 测试增量溯源 + lint 定性，2026-09-11）

> [授权] 架构师微批次授权 2026-09-08（D4g：禁改代码，纯侦查 + 申报；完成即停）。
> [纪律执行] 零代码改动、零测试改动；产物 = 侦查报告
> [`docs/audits/phase4-d4/d4g-recovery-audit.md`](./audits/phase4-d4/d4g-recovery-audit.md)
> + 本条目。**全部处置待裁，未自主修/未自主删。**

## G1 恢复提交内含物盘点

- `git log --stat 50dbe27 -1` 指定对象**不存在**（cat-file fatal ×2 + fsck 零悬空）；
  50dbe27b/55b5a5e8 系丢失原仓提交，内容唯一载体 = 恢复提交 `1fb04b0`（106 文件 = A41/M65）。
- 六类归类：refactor 授权新模块 18 + 授权改动 ~40 + D4 系授权面 ~15（均 ✅）；
  **非授权面（对照用户枚举 D4/S/D4c/D4d/D4e + 在库申报）= ①Phase4-D1/D2/D3/E3a 批次
  文件 ~20（含 4 个 e2e spec）②Wave-2 补测 6 spec ③docs/README.md（失真"250 用例"）
  ④diary.ts 1 行 describe 文案**。来源假说：原仓 Phase4 早期批次交付物经恢复提交携带，
  SESSIONS 报告随原仓丢失（代码内批次标签/ADR-023/024 引用完备）。

## G2 测试增量溯源（107 例全分解）

- 权威基线（vitest JSON 全量复跑 + 双版本静态计数互证）：Phase3-F 373 → 恢复后
  **480** → D4f 后 **484**（55 文件）。
- **基线修正**：D4f 报告"475→479"系并行跑 p11-fresh-chain worker 崩溃丢 5 例
  （幻差 −5），权威为 **480→484**；据实更正，不改历史提交。
- 107 恢复批增量 = D4 系标签 17（p5+9/p5c+2/p7f+2/p4-collections+1/process-manager+1/
  p7h+1/p4e3a⑭+1）+ D1-D3/E3a 27 + p5d 4（§4.3 在库申报）+ **Wave-2 补测 59（无申报）**。
- 授权指令「58」对账：= 475（误报基线）− 417（p7h 头注台账检查点 = 373+44）✓ 精确吻合；
  真值 63 − 5 幻差。开放残差：refactor-review 称 D4e 终态 451 例 vs 本仓恢复结果 480，
  差 29 例原仓口径不可再现，留台账对账。

## G3 lint 8 条定性

- #1-3（articles.controller/service、site-config.service 各 1 条未用导入）=
  **授权文件的恢复损伤**（pagination/mizuki-root 上收后导入残留，用法删除证据齐）。
- #4-8（theme-registry 5 条）= **G1 非授权文件（E3a）自带**（unwrapExpression 上收
  残留 + NotFoundException 出生即未用），随 G4 裁定。
- 8 条全部恢复批引入（d4ebf43 终态 lint 0/0）。

## G4 处置建议（待裁，禁自主执行）

① D1/D2/D3/E3a：**保留 + 补授权**（功能在用、D4 系锚依赖、回滚即功能回退）；
② 补测 59 例：**保留 + 补授权**（对象全为授权单源模块，refactor §6 本建议补测，
删除 = 安全关键模块裸奔）+ §3"未新增测试"表述追加勘误；③ p5d 保留（已申报）；
④ README.md 修订后保留（"250 用例"→484）；⑤ diary.ts 随 D2 追认；⑥ lint #1-3 待批后
删 3 行导入；⑦ #4-8 随 E3a 裁定；⑧ 451→480 残差 29 例留原仓台账对账；⑨ D4f 数账
本报告已更正。
