...
- CORS 白名单默认仅放行 `http://localhost:<面板端口>`（同源面板）。**前后端分离或自定义域名访问**时，在 config.json 增加 `"corsOrigins": ["https://你的域名"]` 逐条并入白名单；非白名单 origin 的请求不携带 `Access-Control-Allow-Origin` 头（浏览器侧拒绝）。
